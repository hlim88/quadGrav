...
\begin{align}
Y_{ab} = Y^{np}_{ab} + \Delta_c \big[(2 \beta_c - \alpha_c) \nabla_a \nabla_b R - \alpha_c (g_{cd,ab} + g_{ab,cd} - g_{cb,ad} - g_{ad,cb}) \widetilde{R}^{cd} \big]
...
\begin{align*}
	...
	\chi\widetilde{\gamma}^{ij}(\partial_i\alpha) \left[
		\partial_j \widetilde{R}_{ab}
		- \frac{1}{2}\left(
			\partial_j \overline{\gamma}_{la} + \partial_a \overline{\gamma}_{jl} - \partial_l \overline{\gamma}_{ja}
		\right)\widetilde{R}_{kb}
		- (a\leftrightarrow b)
	\right]
	+ \alpha (D^i D_i) \widetilde{R}_{ab}
\end{align*}
... could write this out transition from $\overline{\gamma}$ to $\widetilde{\gamma}$ but I do not see how that simplifies the expression significantly ... especially I have terms like $(\partial_a\alpha)(\partial^i\chi)\widetilde{R}_{ib}$ in the final expression]
}
...
n^\aaron{c} \nabla_\aaron{c} \widetilde{R}_{ab} &= - \widetilde{V}_{ab}  \\
n^\aaron{c} \nabla_\aaron{c} \widetilde{V}_{ab} &= -  \frac{1}{\alpha} \partial^i \alpha \left(\partial_i \widetilde{R}_{ab} - \frac{3}{2\chi} \widetilde{R}_{ab} \partial_\aaron{i} \chi \right) - (D^\aaron{i} D_\aaron{i} )_{\textrm{BSSN}} \widetilde{R}_{ab} \nonumber \\
