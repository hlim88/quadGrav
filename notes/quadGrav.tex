...
-\frac{1}{4}g_{\mu\nu}R\;, 	\label{eom:metric}
	...
	\notag\\
	&+\frac{\alpha}{4} g_{\mu\nu}\left(g^{\alpha\beta}g_{\rho\sigma,\alpha\beta}-2Q_{\rho\sigma}(g,\partial g)\right)g^{\eta\rho}g^{\delta\sigma} \widetilde{R}_{\eta\delta}\;. 	\label{eom:ricci_tensor}
\end{align}
...
\begin{align}
\Box R = g^{ab} \partial_a \partial_b R
\end{align} 
Similarly, for Eqn~\ref{eom:ricci_tensor} (now we deal with tensor not a scalar), we have
\begin{align}
\Box \tilde{R}_{ab} = g^{ab} \nabla_a \nabla_b \tilde{R}_{ab}
\end{align}
where $\nabla_a$ is covariant derivative
\TODO{Add more}

\subsubsection{BSSN formulation}
