...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    ...
    subroutine compute_rhs_analysis(rhs, u_pt, v_pt, dxu_pt, dyu_pt, dzu_pt, &
                                   dxv_pt, dyv_pt, dzv_pt, w_pt, par, D_dg)
    implicit none

    CCTK_REAL, dimension(NU_G), intent(in)      :: u_pt,dxu_pt,dyu_pt,dzu_pt    
    ...
    CCTK_REAL, dimension(4,4,4,4),intent(inout) :: D_dg    
    CCTK_REAL, dimension(NU_G), intent(inout)   :: rhs       
    CCTK_INT  :: i

    ! Function arrays and time derivatives
    ! Function arrays and time derivatives
    CCTK_REAL                             :: phir_dot, phic_dot, phim_dot
    CCTK_REAL                             :: pir_dot, pic_dot, pim_dot, G0_dot
    CCTK_REAL                             :: Fpir, Fpic, Fpim, FG0
    CCTK_REAL, dimension(I_1:I_3)         :: Fdphim, Fdphir, Fdphic, FdH0
    CCTK_REAL, dimension(I_1:I_3)         :: dphim_dot, dphir_dot, dphic_dot, dH0_dot
    CCTK_REAL, dimension(I_0:I_3)         :: H_dot, Duphim, Duphir, Duphic
    ...
    CCTK_REAL                             :: phi2, trT, ttK, tZ, d0H0
    ...
    CCTK_REAL :: dphim0, dphir0, dphic0
            
    ! other stuff
    CCTK_REAL          :: pi, sigma0, sigma1, sigma2, sf_m, sf_lam
    CCTK_REAL          :: chi1, chi2, chin, lchi1, lchi2, ltime, ltime1, ltime0, frac
    CCTK_INT           :: debug, rhs_type, gauge_type
...
!          write(0,*)myid,']  calc_rhs: alp2 <=0, Fixing it: alp2,dx',alp2,par(p_dx)
   ...
   if (evolve_geometry .EQ. 1) then
    ...
    chi1 = lchi1
    chi2 = lchi2
    if ((gauge_type.eq.3) .AND. (ltime.gt.ltime0) .AND. (ltime.le.ltime1)) then
      frac = (ltime -ltime0)/ltime1
      chi1 = lchi1*(frac**3)*(6.0*(frac**2) - 15.0*frac + 10)
      chi2 = lchi2*(frac**3)*(6.0*(frac**2) - 15.0*frac + 10)
    end if
...
!   the principal part of the Diab
   ...
   !---the source terms ---------------------------
    ...
    pim_dot = (b1*dphim1 + b2*dphim2 + b3*dphim3)* sigma2 +&
     & alp*(-((dphim1*huu11 + dphim2*huu12 + dphim3*huu13)*tK1) -&
     & (dphim1*huu12 + dphim2*huu22 + dphim3*huu23)*tK2 - (dphim1*huu13&
     & + dphim2*huu23 + dphim3*huu33)*tK3 + duphim0*trG0 + duphim1*trG1&
     & + duphim2*trG2 + duphim3*trG3 - (pim*ttK)/2.)+0.d0
    ...
    pir_dot = (b1*dphir1 + b2*dphir2 + b3*dphir3)* sigma2 +&
     & alp*(phir*(phi2*sf_lam + sf_m**2) - (dphir1*huu11 + dphir2*huu12&
    ...
    pic_dot = (b1*dphic1 + b2*dphic2 + b3*dphic3)* sigma2 +&
     & alp*(phic*(phi2*sf_lam + sf_m**2) - (dphic1*huu11 + dphic2*huu12&
...
!   the scalar fields
   ...
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
   !!!!!!!!!!!!  THE DERIVATIVES OF D0AB  !!!!!!!!!!!!!!!!
   !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    
   D_dg(4,1,1,1) = rhs(H_D1G11); D_dg(4,1,1,2) = rhs(H_D1G12); D_dg(4,1,1,3) = rhs(H_D1G13); D_dg(4,1,1,4) = rhs(H_D1G01) 
   D_dg(4,1,2,1) = rhs(H_D1G12); D_dg(4,1,2,2) = rhs(H_D1G22); D_dg(4,1,2,3) = rhs(H_D1G23); D_dg(4,1,2,4) = rhs(H_D1G02)   
   D_dg(4,1,3,1) = rhs(H_D1G13); D_dg(4,1,3,2) = rhs(H_D1G23); D_dg(4,1,3,3) = rhs(H_D1G33); D_dg(4,1,3,4) = rhs(H_D1G03)   
   D_dg(4,1,4,1) = rhs(H_D1G01); D_dg(4,1,4,2) = rhs(H_D1G02); D_dg(4,1,4,3) = rhs(H_D1G03); D_dg(4,1,4,4) = rhs(H_D1G00)
   
   D_dg(4,2,1,1) = rhs(H_D2G11); D_dg(4,2,1,2) = rhs(H_D2G12); D_dg(4,2,1,3) = rhs(H_D2G13); D_dg(4,2,1,4) = rhs(H_D2G01) 
   D_dg(4,2,2,1) = rhs(H_D2G12); D_dg(4,2,2,2) = rhs(H_D2G22); D_dg(4,2,2,3) = rhs(H_D2G23); D_dg(4,2,2,4) = rhs(H_D2G02)   
   D_dg(4,2,3,1) = rhs(H_D2G13); D_dg(4,2,3,2) = rhs(H_D2G23); D_dg(4,2,3,3) = rhs(H_D2G33); D_dg(4,2,3,4) = rhs(H_D2G03)   
   D_dg(4,2,4,1) = rhs(H_D2G01); D_dg(4,2,4,2) = rhs(H_D2G02); D_dg(4,2,4,3) = rhs(H_D2G03); D_dg(4,2,4,4) = rhs(H_D2G00)
         
   D_dg(4,3,1,1) = rhs(H_D3G11); D_dg(4,3,1,2) = rhs(H_D3G12); D_dg(4,3,1,3) = rhs(H_D3G13); D_dg(4,3,1,4) = rhs(H_D3G01) 
   D_dg(4,3,2,1) = rhs(H_D3G12); D_dg(4,3,2,2) = rhs(H_D3G22); D_dg(4,3,2,3) = rhs(H_D3G23); D_dg(4,3,2,4) = rhs(H_D3G02)   
   D_dg(4,3,3,1) = rhs(H_D3G13); D_dg(4,3,3,2) = rhs(H_D3G23); D_dg(4,3,3,3) = rhs(H_D3G33); D_dg(4,3,3,4) = rhs(H_D3G03)   
   D_dg(4,3,4,1) = rhs(H_D3G01); D_dg(4,3,4,2) = rhs(H_D3G02); D_dg(4,3,4,3) = rhs(H_D3G03); D_dg(4,3,4,4) = rhs(H_D3G00)

   D_dg(4,4,4,4) = alp*rhs(H_K00) + (b1*rhs(H_D1G00) + b2*rhs(H_D2G00) &
  &       + b3*rhs(H_D3G00)) + 0.5d0*alp*ttD0*K00  &
  &       + alp*(huu11*tD01*D100 + huu12*tD01*D200 + huu13*tD01*D300  &
  &       + huu12*tD02*D100 + huu22*tD02*D200 + huu23*tD02*D300  &
  &       + huu13*tD03*D100 + huu23*tD03*D200 + huu33*tD03*D300)

   D_dg(4,4,4,1) = alp*rhs(H_K01) + (b1*rhs(H_D1G01) + b2*rhs(H_D2G01) &
  &       + b3*rhs(H_D3G01)) + 0.5d0*alp*ttD0*K01  &
  &       + alp*(huu11*tD01*D101 + huu12*tD01*D201 + huu13*tD01*D301  &
  &       + huu12*tD02*D101 + huu22*tD02*D201 + huu23*tD02*D301  &
  &       + huu13*tD03*D101 + huu23*tD03*D201 + huu33*tD03*D301)
     
   D_dg(4,4,4,2) = alp*rhs(H_K02) + (b1*rhs(H_D1G02) + b2*rhs(H_D2G02) &
  &       + b3*rhs(H_D3G02)) + 0.5d0*alp*ttD0*K02  &
  &       + alp*(huu11*tD01*D102 + huu12*tD01*D202 + huu13*tD01*D302  &
  &       + huu12*tD02*D102 + huu22*tD02*D202 + huu23*tD02*D302  &
  &       + huu13*tD03*D102 + huu23*tD03*D202 + huu33*tD03*D302)

   D_dg(4,4,4,3) = alp*rhs(H_K03) + (b1*rhs(H_D1G03) + b2*rhs(H_D2G03) &
  &       + b3*rhs(H_D3G03)) + 0.5d0*alp*ttD0*K03  &
  &       + alp*(huu11*tD01*D103 + huu12*tD01*D203 + huu13*tD01*D303  &
  &       + huu12*tD02*D103 + huu22*tD02*D203 + huu23*tD02*D303  &
  &       + huu13*tD03*D103 + huu23*tD03*D203 + huu33*tD03*D303)
     
   D_dg(4,4,1,1) = alp*rhs(H_K11) + (b1*rhs(H_D1G11) + b2*rhs(H_D2G11)&
  &       + b3*rhs(H_D3G11)) + 0.5d0*alp*ttD0*K11  &
  &       + alp*(huu11*tD01*D111 + huu12*tD01*D211 + huu13*tD01*D311  &
  &       + huu12*tD02*D111 + huu22*tD02*D211 + huu23*tD02*D311  &
  &       + huu13*tD03*D111 + huu23*tD03*D211 + huu33*tD03*D311)
   D_dg(4,4,1,2) = alp*rhs(H_K12) + (b1*rhs(H_D1G12) + b2*rhs(H_D2G12) &
  &       + b3*rhs(H_D3G12)) + 0.5d0*alp*ttD0*K12  &
  &       + alp*(huu11*tD01*D112 + huu12*tD01*D212 + huu13*tD01*D312  &
  &       + huu12*tD02*D112 + huu22*tD02*D212 + huu23*tD02*D312  &
  &       + huu13*tD03*D112 + huu23*tD03*D212 + huu33*tD03*D312)
   D_dg(4,4,1,3) = alp*rhs(H_K13) + (b1*rhs(H_D1G13) + b2*rhs(H_D2G13) &
  &       + b3*rhs(H_D3G13)) + 0.5d0*alp*ttD0*K13  &
  &       + alp*(huu11*tD01*D113 + huu12*tD01*D213 + huu13*tD01*D313  &
  &       + huu12*tD02*D113 + huu22*tD02*D213 + huu23*tD02*D313  &
  &       + huu13*tD03*D113 + huu23*tD03*D213 + huu33*tD03*D313)
   D_dg(4,4,2,2) = alp*rhs(H_K22) + (b1*rhs(H_D1G22) + b2*rhs(H_D2G22)&
  &       + b3*rhs(H_D3G22)) + 0.5d0*alp*ttD0*K22  &
  &       + alp*(huu11*tD01*D122 + huu12*tD01*D222 + huu13*tD01*D322  &
  &       + huu12*tD02*D122 + huu22*tD02*D222 + huu23*tD02*D322  &
  &       + huu13*tD03*D122 + huu23*tD03*D222 + huu33*tD03*D322)
   D_dg(4,4,2,3) = alp*rhs(H_K23) + (b1*rhs(H_D1G23) + b2*rhs(H_D2G23) &
  &       + b3*rhs(H_D3G23)) + 0.5d0*alp*ttD0*K23  &
  &       + alp*(huu11*tD01*D123 + huu12*tD01*D223 + huu13*tD01*D323  &
  &       + huu12*tD02*D123 + huu22*tD02*D223 + huu23*tD02*D323  &
  &       + huu13*tD03*D123 + huu23*tD03*D223 + huu33*tD03*D323)
   D_dg(4,4,3,3) = alp*rhs(H_K33) + (b1*rhs(H_D1G33) + b2*rhs(H_D2G33) &
  &       + b3*rhs(H_D3G33)) + 0.5d0*alp*ttD0*K33  &
  &       + alp*(huu11*tD01*D133 + huu12*tD01*D233 + huu13*tD01*D333  &
  &       + huu12*tD02*D133 + huu22*tD02*D233 + huu23*tD02*D333  &
  &       + huu13*tD03*D133 + huu23*tD03*D233 + huu33*tD03*D333)

   D_dg(4,4,1,4) = D_dg(4,4,4,1); D_dg(4,4,2,4) = D_dg(4,4,4,2); D_dg(4,4,3,4) = D_dg(4,4,4,3)
   D_dg(4,4,2,1) = D_dg(4,4,1,2); D_dg(4,4,3,1) = D_dg(4,4,1,3); D_dg(4,4,3,2) = D_dg(4,4,2,3)
    
            
   end subroutine compute_rhs_analysis
