...
subroutine TVDRK3_FV(u0, u2, v, w, dxu,dyu,dzu, dxv, dyv, dzv, urk1, par)
  ...
  CCTK_REAL, parameter                     :: two_third = 0.66666666666666666
...
!=======================================================================
      ...
      if (istat .ne. 0) then
        write(0,*)'>>> RK3TVD_FV: Could not allocate memory for dzu_diss'
        write(0,*)'               size : ',nx,ny,nz
      end if
      allocate(dyu_diss(nx,ny,nz), STAT=istat)
      if (istat .ne. 0) then
        write(0,*)'>>> RK3TVD_FV: Could not allocate memory for dyu_diss'
        write(0,*)'               size : ',nx,ny,nz
      end if
      allocate(dxu_diss(nx,ny,nz), STAT=istat)
      if (istat .ne. 0) then
        write(0,*)'>>> RK3TVD_FV: Could not allocate memory for dxu_diss'
        write(0,*)'                size : ',nx,ny,nz
      end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (P_ENABLE_ALT_COORDS==1) then
        if (ltrace) then
           write(0,*)'>>> RK3TVD_FV: Calling def_alt_coords'
        end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      ...
      if (use_mask .eq. 1) then
        ...
        if (istat .ne. 0) then
          write(0,*)'RK3TVD_FV: Could not allocate memory for imask array'
          write(0,*)'           size : ',nx,ny,nz
        end if

        allocate(imask2(nx,ny,nz),STAT=istat)
        if (istat .ne. 0) then
          write(0,*)'RK3TVD_FV: Could not allocate memory for imask2 array'
          write(0,*)'           size : ',nx,ny,nz
        end if
...
!#######################################################################
    ...
    if (iter .eq. 1) then
       if (ltrace) then
           write(0,*)'>>> RK3TVD_FV: begin first step'
           write(0,*)'>>> RK3TVD_FV: dt,  dx = ',dt, dx
       end if
       if ( ltrace2 ) then
           write(0,*)'>>>RK3TVD_FV: initial data'
           do m=1,NU
       ...
       if ( boundary_conditions .eq. 1 .or.             &
            boundary_conditions .eq. 2 .or.             &
            boundary_conditions .eq. 7 .and. .not.no_bc &
          ) then
            if ( ltraceB ) write(0,*)'>>> RK3TVD_FV: hyperboundary()'
            call hyperboundary(u0, dxu, dyu, dzu, urk1, v, w, imask, par)
       end if

       if ( ltrace  ) write(0,*)'>>> RK3TVD_FV: hyperrhs()'
       if ( ltrace2 ) then
          do m=1,NU
       ...
       if ( ltrace ) then
          write(0,*) 'RK3TVD_FV: iter = ', iter 
       endif
#ifdef HYPER_FLUID
       call SolvePrimVars(v, w, u0, par)
#endif
!      if ( ltrace ) then
!         write(0,*) 'RK3TVD_FV: After 1st SolvePrimVars'
!         write(0,*) 'RK3TVD_FV: u0(H_Sx)%d(1,1,1) = ',u0(H_Sx)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sy)%d(1,1,1) = ',u0(H_Sy)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sz)%d(1,1,1) = ',u0(H_Sz)%d(1,1,1)
!      endif
...
!      if ( ltrace ) then
!         write(0,*) 'RK3TVD_FV: After Calculation of hyperrhs'
!         write(0,*) 'RK3TVD_FV: u0(H_Sx)%d(1,1,1)   = ',u0(H_Sx)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sy)%d(1,1,1)   = ',u0(H_Sy)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sz)%d(1,1,1)   = ',u0(H_Sz)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: urk1(H_Sx)%d(1,1,1) = ',urk1(H_Sx)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: urk1(H_Sy)%d(1,1,1) = ',urk1(H_Sy)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: urk1(H_Sz)%d(1,1,1) = ',urk1(H_Sz)%d(1,1,1)
!      endif
       if ( ltrace2 ) then
          do m=1,NU
       ...
       if (dissipation > 0) then
          if ( ltrace ) write(0,*)'>>> RK3TVD_FV: hyperdissipation()', sigma_diss
          do m=1,NU
       ...
       if ( ltrace2 ) then
           write(0,*)'>>> RK3TVD_FV: after hyperdissipation()'
       endif
       if ( boundary_conditions .gt. 2 .and.  &
            bc_type             .ne. 3 .and.  &
            boundary_conditions .ne. 8 .and. .not.no_bc) then
!          if ( ltraceB ) then  
!               write(0,*)'>>> RK3TVD_FV: hyperboundary'
!               write(0,*)'>>> RK3TVD_FV: dt_gxx = ',urk1(H_G11)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_gxy = ',urk1(H_G12)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_gxz = ',urk1(H_G13)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_gyy = ',urk1(H_G22)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_gyz = ',urk1(H_G23)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_gzz = ',urk1(H_G33)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_Sx  = ',urk1(H_Sx)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_Sy  = ',urk1(H_Sy)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_D   = ',urk1(H_D)%d(1,1,1)
!               write(0,*)'>>> RK3TVD_FV: dt_tau = ',urk1(H_TAU)%d(1,1,1)
!           endif
            call hyperboundary(u0, dxu, dyu, dzu, urk1, v, w, imask, par)
        end if
!      if ( ltrace ) then
!         write(0,*) 'RK3TVD_FV: After hyperboundary'
!         write(0,*) 'RK3TVD_FV: u0(H_Sx)%d(1,1,1) = ',u0(H_Sx)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sy)%d(1,1,1) = ',u0(H_Sy)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sz)%d(1,1,1) = ',u0(H_Sz)%d(1,1,1)
!       endif
        if ( ltrace ) write(0,*)'>>> RK3TVD_FV: update u2()'
        do k = 1, nz
           do j = 1, ny
              do i = 1, nx
...
!      if ( ltrace ) then
!         write(0,*) 'RK3TVD_FV: After update of u2'
!         write(0,*) 'RK3TVD_FV: u0(H_Sx)%d(1,1,1) = ',u0(H_Sx)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sy)%d(1,1,1) = ',u0(H_Sy)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u0(H_Sz)%d(1,1,1) = ',u0(H_Sz)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u2(H_Sx)%d(1,1,1) = ',u2(H_Sx)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u2(H_Sy)%d(1,1,1) = ',u2(H_Sy)%d(1,1,1)
!         write(0,*) 'RK3TVD_FV: u2(H_Sz)%d(1,1,1) = ',u2(H_Sz)%d(1,1,1)
!       endif
        ...
        if (use_mask .eq. 1) then
           if (ltrace) write(0,*)'>>> RK3TVD_FV: use_mask'
...
!         write(0,*) 'RK3TVD_FV: u2(H_Sz)%d(1,1,1) = ',u2(H_Sz)%d(1,1,1)
!         write(0,*) '-----'
!         write(0,*) 'RK3TVD_FV: u0(H_Sx)%d(2,2,2) = ',u0(H_Sx)%d(2,2,2)
!         write(0,*) 'RK3TVD_FV: u0(H_Sy)%d(2,2,2) = ',u0(H_Sy)%d(2,2,2)
!         write(0,*) 'RK3TVD_FV: u0(H_Sz)%d(2,2,2) = ',u0(H_Sz)%d(2,2,2)
!         write(0,*) '- - -'
!         write(0,*) 'RK3TVD_FV: u2(H_Sx)%d(2,2,2) = ',u2(H_Sx)%d(2,2,2)
!         write(0,*) 'RK3TVD_FV: u2(H_Sy)%d(2,2,2) = ',u2(H_Sy)%d(2,2,2)
!         write(0,*) 'RK3TVD_FV: u2(H_Sz)%d(2,2,2) = ',u2(H_Sz)%d(2,2,2)
!         write(0,*) '------'
!         write(0,*) 'RK3TVD_FV: u0(H_Sx)%d(3,3,3) = ',u0(H_Sx)%d(3,3,3)
!         write(0,*) 'RK3TVD_FV: u0(H_Sy)%d(3,3,3) = ',u0(H_Sx)%d(3,3,3)
!         write(0,*) 'RK3TVD_FV: u0(H_Sz)%d(3,3,3) = ',u0(H_Sx)%d(3,3,3)
!         write(0,*) '- - -'
!         write(0,*) 'RK3TVD_FV: u2(H_Sx)%d(3,3,3) = ',u2(H_Sx)%d(3,3,3)
!         write(0,*) 'RK3TVD_FV: u2(H_Sy)%d(3,3,3) = ',u2(H_Sx)%d(3,3,3)
!         write(0,*) 'RK3TVD_FV: u2(H_Sz)%d(3,3,3) = ',u2(H_Sx)%d(3,3,3)
!      endif
#ifdef HYPER_FLUID
        call SolvePrimVars(v, w, u2, par)
#endif
        if (ltrace) write(0,*)'>>> RK3TVD_FV: Done step 1'
...
!#######################################################################
          ...
          if (ltrace) then
              write(0,*)'>>> RK3TVD_FV: begin second step'
          end if
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
       ...
       if ( ltrace ) then
        ...
        call SolvePrimVars(v, w, u2, par) 
#endif
        if (ltrace) write(0,*)'>>> RK3TVD_FV: Done step 2' 
...
!#######################################################################
        ...
        if (ltrace) then
            write(0,*)'>>> RK3TVD_FV: begin third step'
        end if
   ...
   &                                   u2(m)%d,   w,         imask,     par)
        ...
        if (boundary_conditions .gt. 2 .and.            &
   &        bc_type             .ne. 3 .and.            &
   &        boundary_conditions .ne. 8 .and. .not.no_bc &
   &       ) then
             if (ltraceB) write(0,*)'>>> RK3TVD_FV: hyperboundary()'
             call hyperboundary(u2, dxu, dyu, dzu, urk1, v, w, imask, par)
        end if
...
!#######################################################################
    ...
    if (allocated(dxu_diss) .and.  &
        ...
        if (ltrace2) write(0,*)'>>> RK3TVD_FV: about to deallocate diss vars...'
        deallocate(dzu_diss, STAT=istat)
        if (istat .ne. 0) then
          write(0,*)'>>> RK3TVD_FV: Could not deallocate dzu_diss: istat=', istat
        end if
        deallocate(dxu_diss, STAT=istat)
        if (istat .ne. 0) then
          write(0,*)'>>> RK3TVD_FV: Could not deallocate dxu_diss: istat=', istat
        end if
        deallocate(dyu_diss, STAT=istat)
        if (istat .ne. 0) then
          write(0,*)'>>> RK3TVD_FV: Could not deallocate dyu_diss: istat=', istat
        end if
        if (ltrace2) write(0,*)'>>> RK3TVD_FV: ...diss vars deallocated'
    else
        write(0,*)'>>> RK3TVD_FV: diss vars not allocated as expected!'
    end if
    ...
    if (use_mask .eq. 1) then
      if (allocated(imask)) then
        deallocate(imask,imask2)
         ...
         write(0,*)'>>> RK3TVD_FV: imask not allocated as expected!'
      end if
    end if

    if (ltrace) then
       write(0,*)'>>>RK3TVD_FV: return'
       if (ltrace2) then
          do m=1,NU
